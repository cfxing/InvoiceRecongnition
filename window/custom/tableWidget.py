...
class AdaptiveThresholdTableWidget(TableWidget):
    def __init__(self, parent=None):
        super(AdaptiveThresholdTableWidget, self).__init__(parent=parent)
        ...
        self.method_comBox = QComboBox()
        self.method_comBox.addItems(['平均','高斯'])
        self.method_comBox.setObjectName('adaptiveMethod')

        self.type_comBox = QComboBox()
        self.type_comBox.addItems(['二进制阈值化', '反二进制阈值化'])
        self.type_comBox.setObjectName('thresholdType')

        self.setColumnCount(2)
        self.setRowCount(3)

        self.setItem(0, 0, QTableWidgetItem('方法'))
        self.setCellWidget(0, 1, self.method_comBox)
        self.setItem(1, 0, QTableWidgetItem('类型'))
        self.setCellWidget(1, 1, self.type_comBox)
        self.setItem(2, 0, QTableWidgetItem('最大值'))
        self.setCellWidget(2, 1, self.maxval_spinBox)
...
class GammaTableWidget(TableWidget):
    def __init__(self, parent=None):
        super(GammaTableWidget, self).__init__(parent=parent)
