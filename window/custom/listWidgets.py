...
class ShowResWidget(QTextEdit):
    def __init__(self, parent=None):
        super(ShowResWidget, self).__init__(parent=parent)
...
class FuncListWidget(MyListWidget):
    ...
    def add_used_function(self):
        ...
        if type(func_item) in items:
            ...
            # print(type(use_item))
            # print(isinstance(use_item,items[-1]))
            self.mainwindow.useListWidget.addItem(use_item)
            self.mainwindow.update_image()
            # 得到 cnts
            if isinstance(use_item,items[-3]):
                self._cnts = use_item.getCnts()
