...
from train_ctpn.ctpn_model import CTPN_Model
from train_ctpn.ctpn_utils import gen_anchor, bbox_transfor_inv, clip_box, filter_bbox, nms, TextProposalConnectorOriented
from train_ctpn.ctpn_utils import resize
import config
...
def get_det_boxes(image, h,display=True,expand=True):
    image = resize(image, height=int(2.8 * h))
    image_r = image.copy()
    image_c = image.copy()
    h, w = image.shape[:2]
    ...
    print('changedImgShape',image.shape)
