...
def gen_anchor(feature_size, scale):
    """
...
def bbox_transfrom(anchors, gt_boxes):
    """
...
def cal_rpn(img_size, feature_size, scale, gt_boxes, OHEM=None):
    img_h, img_w = img_size

    # gen base anchor
    base_anchor = gen_anchor(feature_size, scale)

    # calculate iou
    overlaps = cal_overlaps(base_anchor, gt_boxes)
    ...
    # debug here
    bbox_targets = bbox_transfrom(base_anchor, gt_boxes[anchor_argmax_overlaps, :])
    # bbox_targets=[]
